...
dataM <- reactive({
		if(input$dataInput==1){
			...
			var <- input$ggrouping
			data <- dcast(melt(data, id.vars=c(input$ggrouping), measure.vars=c(input$gdataID)), as.formula(paste0("value~",var))) #NOTE replace 'region' with a variable, and '3' with a range variable from input
